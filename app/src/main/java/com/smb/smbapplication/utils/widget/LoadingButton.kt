...
class LoadingButton : View {
    ...
    private fun init(context: Context, attrs: AttributeSet?) {
        ...
        if (boldText) {
            mTextPaint!!.typeface = ResourcesCompat.getFont(context, R.font.metropolis_medium)
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        when (currentState) {
            ...
            STATE_ANIMATION_LOADING -> {
                mPath!!.reset()
                mArcRectF?.let { mPath!!.addArc(it, (270 + mAngle / 2).toFloat(), (360 - mAngle).toFloat()) }
                if (mAngle != 0) {
            ...
            STATE_STOP_LOADING -> {
                mPath!!.reset()
                this!!.mArcRectF?.let { mPath!!.addArc(it, (270 + mAngle / 2).toFloat(), mEndAngle.toFloat()) }
                if (mEndAngle != 360) {
                    mMatrix.setRotate(mDegree.toFloat(), (width / 2), (height / 2))
                    mPath!!.transform(mMatrix)
                    mDegree += 10
